(function (win, dependencies, components) {
  ...
  function watchChatRoom(status) {
    console.log('chatroom变更', status)
    vueInstance.addOutput('chatroom变更', status, 0, [], {
      color: utils.TypeColor.MSG
    });
  }

  function watchBlockedMessage(status) {
    console.log('收到屏蔽消息回调', status)
    vueInstance.addOutput('收到屏蔽消息回调', status, 0, [], {
      color: utils.TypeColor.MSG
    });
  }
